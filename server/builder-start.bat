...
if %errorlevel% equ 0 (
    ...
    echo 创建新的编译容器...
    docker run -d --name warforge-builder --entrypoint sh --network warforge-network -p 8201:8201 -v "%cd%":/app -w /app heroiclabs/nakama-pluginbuilder:latest -c "sleep infinity"
)
...
echo 端口映射: 8201 -^> 8201 (WebAdmin API)

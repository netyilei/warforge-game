...
export class BehaviorTree {
    ...
    public recordPath(path:string){
        this._recordPaths.push(path);
    }
    public B3Log(...args: any[]){
        if(this.isOpenLog){
            Log.oth.info("[BehaviorTree]",...args)
        }
    }
    ...
    public load(data: any, names: any) {
        ...
        for (let id in data.nodes) {
            ...
            let node: BaseNode = new Cls(spec.properties);
    ...
    public tick(target: object, blackboard: Blackboard<any>): b3.Status {
        this._recordPaths = [];
        ...
        this.B3Log("[ActionPath]",this._recordPaths.join("-->"))
        return state;
    }
}
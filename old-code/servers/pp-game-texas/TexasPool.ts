	...
	checkFinished() {
		...
		for(let user of this.users) {
			...
			if(!value) {
				...
				return false 
			} else if(user.betType > TexasDefine.BetType.Allin) {
				return false 
			}
		}
		this.finished = true 
...
export class TexasPoolManager {
	...
	onBet(chairNo:number,value:Decimal,betType:TexasDefine.BetType) {
		...
		Log.oth.info("onbet end = ",betType)
		return null 
	}

	onUserType(chairNo:number,betType:TexasDefine.BetType) {
		let userType = this.userTypes.find(v=>v.chairNo == chairNo)
		if(!userType) {	
			userType = {
				chairNo,
				lastType:betType,
			...
			userType.lastType = betType
			userType.phaseType = betType
		}
	}
	...
	fromSync(sync:TexasUserMsg.tTexasPoolManagerSync) {
		...
		this.phasePoolStartIdx = this.phasePoolStartIdx

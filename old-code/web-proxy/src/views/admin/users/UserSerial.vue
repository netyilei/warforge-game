                        ...
                        <CTable hover responsive>
                            <CTableHead>
                                <CTableRow>
                                    ...
                                    <CTableHeaderCell>设置</CTableHeaderCell>
                            ...
                            <CTableBody>
                                <CTableRow v-for="serial in serialList" :key="serial.no">
                                    ...
                                    <CTableDataCell>
                                        <CBadge v-if="serial.setup" color="info">是</CBadge>
...
import { userGetserials } from '@/web/ProxyReq'
...
const loadSerials = async () => {
    loading.value = true
    try {
        const params: ProxyReqDefine.tUserGetserialsReq = {
...
// 获取流水类型名称
const getSerialTypeName = (type: number): string => {
...
// 获取流水类型颜色
const getSerialTypeColor = (type: number): string => {
...
onMounted(() => {
    ...
    if (userIDFromQuery !== undefined && userIDFromQuery !== null) {
        const id = Number(userIDFromQuery)
        if (!Number.isNaN(id)) {
            filter.value.userID = id
        }
    }
    loadSerials()
})

...
@ccclass
@menu('cpp/lobby/widgets/WithdrawWidget_Paypal')
export default class WithdrawWidget_Paypal extends cc.Component {
	@property(cc.Label)
	...
	private configs_:ChargeDefine.tWithdrawPaypalConfig[]
	private curConfig_:ChargeDefine.tWithdrawPaypalConfig
	onFocus() {
	...
	async initData() {
		...
		this.configs_ = res.paypals
		this.curConfig_ = this.configs_[0]
		if(!this.curConfig_) {
			return 
 		}
	...
	onClickConfirm() {
		...
		let email = this.editEmail.string.trim()
		if(email.length === 0) {
			kcore.tip.push("提示","请输入Paypal邮箱地址")
			return
		}

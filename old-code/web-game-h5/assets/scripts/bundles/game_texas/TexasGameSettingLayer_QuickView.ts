...
export default class TexasQuickSetView extends cc.Component {
	@property({ type: [ButtonCheckBox] })
	buttonCounts: ButtonCheckBox[] = [];
	...
	protected onLoad(): void {
		...
		let selectConfigIndex = this.configs.findIndex(v => v.buttonCount == this.quickBetSelect)
		for(let i = 0 ; i < this.buttonCounts.length ; i ++) {
			let check = this.buttonCounts[i]
			check.setFunc(()=>{
				let config = this.configs[i]
				this.onClickCount(null, config.buttonCount.toString())
				for(let other of this.buttonCounts) {
					other.isChecked = other == check 
				}
			})
			check.isChecked = i == selectConfigIndex
		}
	}
	...
	setSlider() {
		...
		this.progress.bar.progress = progress

		let com = this.betButtons[this.selectBetIndex]
		com.set(value, this.sliderLbl)
	}
	private prevProgress_: number = -1
	update() {
		if (this.prevProgress_ == this.progress.bar.progress) {
			return
		}
		this.prevProgress_ = this.progress.bar.progress
		//更新滑动条上的数值限制提示
		let config = this.getConfig()
		let min: number = config.betValues[this.selectBetIndex * 2]
		let max: number = config.betValues[this.selectBetIndex * 2 + 1]

		let value = min + (max - min) * this.progress.bar.progress
	...
	protected setPolygon(item: cc.Node) {
		let pos = kcore.utils.convertPosition(item,this.polygon)
		this.polygon.active = false
		this.polygon.x = pos.x

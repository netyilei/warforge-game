...
import { loginConsole } from '@/web/LoginReq'
...
const submit = async () => {
  ...
  loading.value = true
  try {
    // 构造登录参数（根据AdminLoginReqDefine.ts中的定义）
    const loginReq = {
    ...
    const res = await loginConsole(loginReq)
    ...
    // 保存登录信息与当前 Console 下的角色列表（用于前端权限控制）
    sessionStorage.setItem('account', JSON.stringify(res.loginData || {}))
    sessionStorage.setItem('ak', res.ak)
    sessionStorage.setItem('loginData', JSON.stringify(res.loginData || {}))
    sessionStorage.setItem('roleTarget', JSON.stringify(res.roleTarget || { target: 1, roles: [] }))
    sessionStorage.setItem('customerHost', res.customerHost || '')
    sessionStorage.setItem('customerWSHost', res.customerWSHost || '')
...
.login-wrapper {
  position: relative;
  min-height: 100vh;
  background: url('/game-admin/back.jpg') center center / cover no-repeat;
}

<template>
    <CRow>
                        ...
                        <CTable hover responsive>
                            <CTableHead>
                                <CTableRow>
                                    ...
                                    <CTableHeaderCell>充值返利</CTableHeaderCell>
                            ...
                            <CTableBody>
                                <CTableRow v-for="user in userList" :key="user.userID"
                                    ...
                                    <CTableDataCell>{{ getProxyChargeField(user) }}</CTableDataCell>
                                    ...
                                    <CTableDataCell>
                                        <CDropdown variant="btn-group">
                                            ...
                                            <CDropdownMenu>
                                                ...
                                                <CDropdownItem @click="openProxyChargeModal(user)">
                                                    设置代理充值奖励
                                                </CDropdownItem>
                                                <CDropdownItem @click="openChangePasswordModal(user.userID)">
                                                    修改密码
                                                </CDropdownItem>
                                                <CDropdownItem v-if="isUserProxy(user)" @click="setUserToProxy(user,false)">
                                                    降级为普通用户
                                                </CDropdownItem>
        ...
        <CModal :visible="showCreateAccountModal" @close="closeCreateAccountModal">
            <CModalHeader>
                <CModalTitle>创建代理账户</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
        ...
        <!-- 设置代理充值奖励 Modal -->
        <CModal :visible="showProxyChargeModal" @close="closeProxyChargeModal">
            <CModalHeader>
                <CModalTitle>设置代理充值奖励</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
                        <CFormLabel>下级充值奖励百分比</CFormLabel>
                        <CFormInput
                            type="number"
                            v-model.number="proxyChargeForm.charges[0].chargeLeaderPercent"
                            placeholder="输入返利百分比"
                        />
                    </div>
                </CForm>
            ...
            <CModalFooter>
                <CButton color="secondary" @click="closeProxyChargeModal">取消</CButton>
                <CButton color="primary" @click="saveProxyCharge" :disabled="saving">
                    {{ saving ? '保存中...' : '保存' }}
                </CButton>
            </CModalFooter>
        </CModal>
...
const userList = ref<AdminReqDefine.tUserFilterGetproxyusersRes['users']>([])
...
const showProxyChargeModal = ref(false)
const proxyChargeForm = ref<RewardDefine.tProxyCharge>({
    userID: 0,
    charges: [],
})
...
const getGoldBalance = (user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number]): string => {
...
const getBalanceField = (
    user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number],
...
// 平衡期望值 > 0 时用于行高亮
const isExceptionValuePositive = (user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number]): boolean => {
    // let exceptionValue = new Decimal(user.balance?.exceptionValue || 0)
    // let totalCharge = new Decimal(user.balance?.totalCharge || 0)
    // let win = new Decimal(user.balance?.win || 0)
    // let lose = new Decimal(user.balance?.lose || 0)
...
const getProxyChargeField = (
    user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number],
): string => {
    let val = user.proxyCharge?.charges?.find(v=>v.itemID == ItemID.Gold)?.chargeLeaderPercent
	if(val == null) {
		val = chargeConfig.value.charges?.find(v=>v.itemID == ItemID.Gold)?.chargeLeaderPercent || 0
		return val + "%(全局)"
	}
	return val + "%" 
}
...
const loadUsers = async () => {
    loading.value = true
    try {
        const params: AdminReqDefine.tUserFilterGetproxyusersReq = {
        ...
        const response = await userFilterGetproxyusers(params)
...
const openProxyChargeModal = (user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number]) => {
	let charges = user.proxyCharge?.charges || []
	if(charges.length == 0) {
		charges = [{
			itemID: ItemID.Gold,
			minChargeValue: '0',
	...
	proxyChargeForm.value = {
		userID: user.userID,
		charges: charges,
	}
	showProxyChargeModal.value = true
}

const closeProxyChargeModal = () => {
    showProxyChargeModal.value = false
}

const saveProxyCharge = async () => {
	const response = await configSetproxycharge({
		charge: proxyChargeForm.value,
	})

	if (response.errCode) {
		Toast.error(response.errMsg || '保存充值奖励失败')
		return
	}
	Toast.success('保存充值奖励成功')
	closeProxyChargeModal()
	await loadUsers()
}
...
const isUserProxy = (user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number]): boolean => {
...
const setUserToProxy = async (user: AdminReqDefine.tUserFilterGetproxyusersRes['users'][number], b: boolean) => {

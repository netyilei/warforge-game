<template>
    <CRow>
            ...
            <CCard class="mb-4">
                ...
                <CCardBody>
                    ...
                    <CRow class="mb-3">
                        ...
                        <CCol :md="3">
                            <CFormLabel>登录渠道</CFormLabel>
                            <select class="form-select" v-model.number="filterChannel">
                                <option :value="undefined">全部渠道</option>
                                    ...
                                    <CTableDataCell>
                                        <CDropdown variant="btn-group">
                                            ...
                                            <CDropdownMenu>
                                                ...
                                                <CDropdownItem @click="addUserClubValue(user.userID)">
                                                    添加俱乐部价值
                                                </CDropdownItem>
                                                <CDropdownItem @click="openChangePasswordModal(user.userID)">
                                                    修改密码
                                                </CDropdownItem>
                                                <CDropdownItem @click="openChangeLoginTargetModal(user.userID, user.loginRole)">
                                                    登录平台
                                                </CDropdownItem>
            ...
            <CModalBody>
                <CForm>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" v-model.number="createAccountForm.target">
        ...
        <!-- 更改登录平台 Modal -->
        <CModal :visible="showChangeLoginTargetModal" @close="closeChangeLoginTargetModal">
            <CModalHeader>
                <CModalTitle>更改登录平台 (用户ID: {{ changeLoginTargetForm.userID }})</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
                        <CFormLabel>登录平台</CFormLabel>
                        <select class="form-select" v-model.number="changeLoginTargetForm.target">
                            <option :value="UserDefine.LoginTarget.App">App</option>
                            <option :value="UserDefine.LoginTarget.Console">Console</option>
                            <option :value="UserDefine.LoginTarget.H5">H5</option>
                            <option :value="UserDefine.LoginTarget.WechatMini">微信小程序</option>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" v-model="changeLoginTargetForm.b">
            ...
            <CModalFooter>
                <CButton color="secondary" @click="closeChangeLoginTargetModal">取消</CButton>
                <CButton color="primary" @click="saveChangeLoginTarget" :disabled="saving">
                    {{ saving ? '保存中...' : '保存' }}
                </CButton>
            </CModalFooter>
        </CModal>
...
const filterChannel = ref<UserDefine.LoginChannel | undefined>(undefined)
...
const getGoldBalance = (user: AdminReqDefine.tUserGetusersRes['users'][number]): string => {
...
const getBalanceField = (
    user: AdminReqDefine.tUserGetusersRes['users'][number],
...
// 平衡期望值 > 0 时用于行高亮
const isExceptionValuePositive = (user: AdminReqDefine.tUserGetusersRes['users'][number]): boolean => {
    let exceptionValue = new Decimal(user.balance?.exceptionValue || 0)
    let totalCharge = new Decimal(user.balance?.totalCharge || 0)
    let win = new Decimal(user.balance?.win || 0)
    let lose = new Decimal(user.balance?.lose || 0)
...
const loadUsers = async () => {
    ...
    try {
        ...
        if (filterChannel.value !== undefined && filterChannel.value !== null) {
...
// Open change login target modal
const openChangeLoginTargetModal = (userID: number, loginRole?: UserDefine.tLoginRole) => {
    changeLoginTargetForm.value = {
        userID: userID,
        target: UserDefine.LoginTarget.Console,
        b: true,
    }
    currentLoginRole.value = loginRole || null
    showChangeLoginTargetModal.value = true
}

// Close change login target modal
const closeChangeLoginTargetModal = () => {
    showChangeLoginTargetModal.value = false
    currentLoginRole.value = null
}

// Save change login target
const saveChangeLoginTarget = async () => {
    saving.value = true
    try {
        const response = await userChangelogintarget(changeLoginTargetForm.value)

        if (response.errCode) {
            Toast.error(response.errMsg || '更改登录平台失败')
            return
        }

        Toast.success('更改登录平台成功')
        closeChangeLoginTargetModal()
        await loadUsers()
    } catch (error) {
        console.error('Change login target error:', error)
        Toast.error('网络错误，请稍后重试')
    } finally {
        saving.value = false
    }

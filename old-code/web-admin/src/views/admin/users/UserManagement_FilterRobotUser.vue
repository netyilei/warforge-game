<template>
    <CRow>
            ...
            <CCard class="mb-4">
                ...
                <CCardBody>
                    ...
                    <CRow class="mb-3">
                        ...
                        <CCol :md="3">
                            <CFormLabel>匹配</CFormLabel>
                            <select class="form-select" v-model.number="filterGroupID">
                                <option :value="undefined">请选择匹配</option>
                                <option v-for="group in groupDatas" :key="group.groupID" :value="group.groupID">
                                    {{ group.groupID }}
                                </option>
                            </select>
                        </CCol>
                        ...
                        <CTable hover responsive>
                            <CTableHead>
                                <CTableRow>
                                    ...
                                    <CTableHeaderCell>专用匹配ID</CTableHeaderCell>
                            ...
                            <CTableBody>
                                <CTableRow v-for="user in userList" :key="user.userID"
                                    ...
                                    <CTableDataCell>{{ user.search.robotGroupID || '-' }}</CTableDataCell>
        ...
        <CModal :visible="showCreateAccountModal" @close="closeCreateAccountModal">
            <CModalHeader>
                <CModalTitle>创建机器人</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
                        <CFormLabel>创建数量</CFormLabel>
                        <CFormInput
                            v-model="createAccountForm.count"
                            placeholder="请输入创建数量"
                        />
                    </div>
                    <div class="mb-3">
                        <CFormLabel>匹配</CFormLabel>
                        <select class="form-select" v-model.number="createAccountForm.groupID">
                            <option :value="undefined">请选择匹配</option>
                            <option v-for="group in groupDatas" :key="group.groupID" :value="group.groupID">
                                {{ group.groupID }}
                            </option>
                    ...
                    <!-- <div class="mb-3">
                        <CFormLabel>匹配</CFormLabel>
                        <select class="form-select" v-model.number="createAccountForm.matchID">
                            <option :value="undefined">请选择匹配</option>
                            <option v-for="match in matchDatas" :key="match.matchID" :value="match.matchID">
                                {{ match.matchID }}
                            </option>
                        </select>
                    </div> -->
...
const userList = ref<AdminReqDefine.tUserFilterGetrobotusersRes['users']>([])
...
const groupDatas = ref<GroupDefine.tData[]>([])
...
const createAccountForm = ref<AdminReqDefine.tRobotExtCreaterobotReq>({
    count: 0,
    groupID: undefined,
    matchID: undefined,
})
...
const getGoldBalance = (user: AdminReqDefine.tUserFilterGetrobotusersRes['users'][number]): string => {
...
const getBalanceField = (
    user: AdminReqDefine.tUserFilterGetrobotusersRes['users'][number],
...
// 平衡期望值 > 0 时用于行高亮
const isExceptionValuePositive = (user: AdminReqDefine.tUserFilterGetrobotusersRes['users'][number]): boolean => {
    // let exceptionValue = new Decimal(user.balance?.exceptionValue || 0)
    // let totalCharge = new Decimal(user.balance?.totalCharge || 0)
    // let win = new Decimal(user.balance?.win || 0)
    // let lose = new Decimal(user.balance?.lose || 0)
...
const loadUsers = async () => {
    loading.value = true
    try {
        const params: AdminReqDefine.tUserFilterGetrobotusersReq = {
        ...
        const response = await userFilterGetrobotusers(params)
...
const saveCreateAccount = async () => {
    if (!createAccountForm.value.count || createAccountForm.value.count <= 0) {
        Toast.warning('请输入创建数量')
        return
    }

    saving.value = true
    try {
        const response = await robotExtCreaterobot(createAccountForm.value)

        if (response.errCode) {
            Toast.error(response.errMsg || '创建机器人失败')
            return
        }

        Toast.success('创建机器人成功')
...
const loadGroupData = async () => {
    ...
    if (response.errCode) {
        Toast.error(response.errMsg || '加载匹配数据失败')
        return
    }
    groupDatas.value = response.groups
}

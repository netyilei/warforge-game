<template>
    <CRow>
        <CCol :xs="12">
            <CCard class="mb-4">
                ...
                <CCardBody>
                    <CRow class="mb-3">
                        <CCol :md="4">
                            <CFormInput v-model="filterChainIDs" placeholder="链ID筛选，逗号分隔（留空全部）" />
                        </CCol>
                        <CCol :md="2">
                        ...
                        <CTable hover responsive>
                            <CTableHead>
                                <CTableRow>
                                    ...
                                    <CTableHeaderCell>RPC URL</CTableHeaderCell>
                                    <CTableHeaderCell>操作</CTableHeaderCell>
                                </CTableRow>
                            </CTableHead>
                            <CTableBody>
                                <CTableRow v-for="row in list" :key="row.chainID">
                                    ...
                                    <CTableDataCell><span class="text-truncate d-inline-block" style="max-width:120px">{{ row.rpcURL }}</span></CTableDataCell>
                                    <CTableDataCell>
                                        <CButton color="primary" size="sm" @click="edit(row)">编辑</CButton>
                                    </CTableDataCell>
                                </CTableRow>
                                <CTableRow v-if="!list.length">
                                    <CTableDataCell colspan="6" class="text-center text-muted py-4">暂无数据</CTableDataCell>
                                </CTableRow>
                            </CTableBody>
                        </CTable>
        ...
        <CModal :visible="showModal" @close="closeModal" size="lg">
            <CModalHeader>
                <CModalTitle>{{ editing ? '编辑' : '新增' }}链信息</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
                        <CFormLabel>链ID <span v-if="!editing" class="text-muted">（新增时必填）</span></CFormLabel>
                        <CFormInput v-model.number="form.chainID" type="number" :disabled="!!editing" placeholder="新增时填写新链ID" />
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <CFormLabel>RPC URL</CFormLabel>
                        <CFormInput v-model="form.rpcURL" placeholder="RPC URL" />
                    </div>
                </CForm>
            </CModalBody>
            <CModalFooter>
...
import { chargeGetchaininfos, chargeUpdatechaininfo } from '@/web/AdminReq'
...
const list = ref<AdminReqDefine.tChargeGetchaininfosRes['datas']>([])
const totalCount = ref(0)
const currentPage = ref(0)
const pageSize = 20
const totalPages = computed(() => Math.ceil(totalCount.value / pageSize))
const filterChainIDs = ref('')
const showModal = ref(false)
const editing = ref<typeof list.value[0] | null>(null)
const form = ref({ chainID: 0, name: '', displayName: '', iconUrl: '', rpcURL: '' })

const loadList = async () => {
    loading.value = true
    try {
        const chainIDs = filterChainIDs.value.trim() ? filterChainIDs.value.split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => !isNaN(n)) : undefined
        const res = await chargeGetchaininfos({ page: currentPage.value, limit: pageSize, chainIDs })
        if (res.errCode) {
...
const add = () => {
    editing.value = null
    form.value = { chainID: 0, name: '', displayName: '', iconUrl: '', rpcURL: '' }
    showModal.value = true
}

const edit = (row: typeof list.value[0]) => {
    editing.value = row
    form.value = { chainID: row.chainID, name: row.name || '', displayName: row.displayName || '', iconUrl: row.iconUrl || '', rpcURL: row.rpcURL || '' }
    showModal.value = true
}
...
const save = async () => {
    saving.value = true
    try {
        const res = await chargeUpdatechaininfo({ data: { ...form.value } })
        if (res.errCode) {

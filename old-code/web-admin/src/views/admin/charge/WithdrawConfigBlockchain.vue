<template>
    <CRow>
        ...
        <CModal :visible="showModal" @close="showModal = false" size="lg">
            <CModalHeader>
                <CModalTitle>{{ isCreate ? '新增' : '编辑' }}区块链提现配置</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <CRow>
                    ...
                    <CRow>
                        ...
                        <CCol :md="6">
                            <div class="mb-3">
                                ...
                                <CFormInput v-model="form.name" placeholder="名称(name)" />
                            </div>
                        </CCol>
                    </CRow>
                    <CRow>
...
import { chargeGetchaininfos, chargeGetwithdrawconfig_blockchain, chargeUpdatewithdrawconfig_blockchain, chargeSetwithdrawconfigenabled_blockchain } from '@/web/AdminReq'
...
const list = ref<ChargeDefine.tWithdrawBlockchainConfig[]>([])
...
const defaultForm = (): ChargeDefine.tWithdrawBlockchainConfig => ({
    typeID: 0,
    enabled: true,
    chainID: 0,
    name: '',
...
const form = ref<ChargeDefine.tWithdrawBlockchainConfig>(defaultForm())
...
const loadList = async () => {
    loading.value = true
    try {
        const res = await chargeGetwithdrawconfig_blockchain({ withDisabled: withDisabled.value, page: currentPage.value, limit: pageSize })
        if (res.errCode) {
...
const edit = (row: ChargeDefine.tWithdrawBlockchainConfig) => {
...
const save = async () => {
    saving.value = true
    try {
        const res = await chargeUpdatewithdrawconfig_blockchain({ create: isCreate.value, data: { ...form.value } })
        if (res.errCode) {
...
const toggleEnabled = async (row: ChargeDefine.tWithdrawBlockchainConfig, enabled: boolean) => {
    const res = await chargeSetwithdrawconfigenabled_blockchain({ typeID: row.typeID, enabled })
    if (res.errCode) {

<template>
    <CRow>
        <CCol :xs="12">
            <CCard class="mb-4">
                ...
                <CCardBody>
                    <CRow class="mb-3">
                        <CCol :md="4">
                            <CFormInput v-model="filterBankIDs" placeholder="银行ID筛选，逗号分隔（留空全部）" />
                        </CCol>
                        <CCol :md="2">
                    ...
                    <div v-else>
                        <CTable hover responsive>
                            ...
                            <CTableBody>
                                <CTableRow v-for="row in list" :key="row.bankID">
        ...
        <CModal :visible="showModal" @close="closeModal" size="lg">
            <CModalHeader>
                <CModalTitle>{{ editing ? '编辑' : '新增' }}银行信息</CModalTitle>
            </CModalHeader>
            <CModalBody>
                <CForm>
                    <div class="mb-3">
                        <CFormLabel>银行ID <span v-if="!editing" class="text-muted">（新增时必填）</span></CFormLabel>
                        <CFormInput v-model.number="form.bankID" type="number" :disabled="!!editing" placeholder="新增时填写新银行ID" />
                    </div>
                    <div class="mb-3">
...
import { chargeGetbankinfos, chargeUpdatebankinfo } from '@/web/AdminReq'
...
const list = ref<AdminReqDefine.tChargeGetbankinfosRes['datas']>([])
...
const form = ref({ bankID: 0, name: '', displayName: '', iconUrl: '' })

const loadList = async () => {
    loading.value = true
    try {
        const bankIDs = filterBankIDs.value.trim() ? filterBankIDs.value.split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => !isNaN(n)) : undefined
        const res = await chargeGetbankinfos({ page: currentPage.value, limit: pageSize, bankIDs })
        if (res.errCode) {
...
const add = () => {
    editing.value = null
    form.value = { bankID: 0, name: '', displayName: '', iconUrl: '' }
    showModal.value = true
}

const edit = (row: typeof list.value[0]) => {
    editing.value = row
    form.value = { bankID: row.bankID, name: row.name || '', displayName: row.displayName || '', iconUrl: row.iconUrl || '' }
    showModal.value = true
}
...
const save = async () => {
    saving.value = true
    try {
        const res = await chargeUpdatebankinfo({ data: { ...form.value } })
        if (res.errCode) {

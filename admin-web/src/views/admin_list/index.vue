...
const fetchData = async () => {
  loading.value = true;
  try {
    const { data: res, error } = await adminApi.getAdminUsers();
    if (error) {
...
const getAdminUserRoles = async (userId: string): Promise<string[]> => {
  try {
    const { data: res, error } = await adminApi.getAdminUserRoles(userId);
    if (error) {
      return [];
    }
    return res?.roles || [];
...
const handleDelete = async (id: string) => {
  try {
    const { error } = await adminApi.deleteAdminUser(id);
    if (error) {
      message.error('删除失败');
      return;
    }
...
const handleSubmit = async () => {
  try {
    if (formData.value.id) {
      const { error } = await adminApi.updateAdminUser(formData.value.id, {
      ...
      if (formData.value.roles && formData.value.roles.length > 0) {
        await adminApi.updateAdminUserRoles(formData.value.id, formData.value.roles);
      }
    } else {

import { effectScope, onScopeDispose, reactive, shallowRef, watch } from 'vue';
...
export function useNaiveTable<ResponseData, ApiData>(options: UseNaiveTableOptions<ResponseData, ApiData, false>) {
  ...
  const scrollX = result.columns.value.reduce((acc, column) => {
...
export function useNaivePaginatedTable<ResponseData, ApiData>(
  ...
  const pagination = reactive({
    ...
    prefix: showTotal
      ? (page: { startIndex: number; endIndex: number; page: number; pageSize: number; pageCount: number; itemCount: number | undefined }) =>
        `共 ${page.itemCount ?? 0} 条`
      : undefined,
